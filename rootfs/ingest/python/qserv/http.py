...
def _get_retry_object(retries=5, backoff_factor=0.2):
...
class Http():
    ...
    def put(self, url, payload=None, timeout=None) -> dict():
        ...
        if not payload:
            payload = {}
        payload["auth_key"] = authKey
        r = self.http.put(url, json=payload, timeout=timeout)
        if (r.status_code != 200):
    ...
    def delete(self, url, timeout=None):
        authKey = authorize()
        r = self.http.delete(url, json={"auth_key": authKey}, timeout=timeout)
        if (r.status_code != 200):
